...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: {file_upload} has been Uploaded successfully to TeamDrive (📝Join TD to view) <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}", reply_markup=button_markup)
        ...
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: Folder has been Uploaded successfully to {tt} in TeamDrive(📝Join TD to view) <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}", reply_markup=button_markup)
        #await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
